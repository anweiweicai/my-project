...
public class FlowUtils {//限流
     ...
     * 针对于在时间段内多次请求限制
     * @param counterKey 计数键
     * @param freequency 请求频率
     * @param period 计数周期
     * @return 是否通过限流检查
     */
    public boolean limitPeriodCounterCheck(String counterKey, int freequency, int period) {//计数器限流
        return this.internalCheck(counterKey, freequency, period, defaultAction);
    }
    /**
     * 内部使用请求限制主要逻辑
     * @param key 计数键
    ...
    private boolean internalCheck(String key, int frequency, int period, LimitAction action) {
        if (Boolean.TRUE.equals(template.hasKey(key))) {
            ...
            // 这行代码使用了Optional类来处理template.opsForValue().increment(key)的返回值。如果返回值不为null，则使用返回值，否则使用0L作为默认值。然后将结果赋给变量value。
